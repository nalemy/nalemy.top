...
后者的必要性是显然的，现在证明其充分性。设存在左部点 $k$ 不在最大匹配中，构造点集 $S_0=\{k\}$ 和 $S_t=\{\hat v\mid v\in \cup_{u\in S_{t-1}}N(u)\}\cup\{k\}$（考虑到，对于任意 $u\in S_{t-1}$，任意 $v\in N(u)$ 均有与它匹配的左部点，否则总能按照集合扩展的路径构造出一条增广路使得答案增加）。
